...
const roleConfigs = [
  {
    ...
    objects: readOnly, classes: ["Resource360Service", "Resource360TalentService", "Resource360PlanningService", "Resource360AssuranceService"], customPermissions: [], editable: {}
  },
  {
...
writePermissionSet({ name: "Resource360_Administrator", label: "Resource 360 Administrator", description: "Full Resource 360 administration. Audit deletion/update remains blocked by immutable triggers.", objects: adminObjects, classes: fs.readdirSync(path.join(metadataRoot,"classes")).filter((name)=>name.endsWith(".cls")).map((name)=>name.replace(".cls","")), customPermissions: fs.readdirSync(path.join(metadataRoot,"customPermissions")).map((name)=>name.replace(".customPermission-meta.xml","")), editable: Object.fromEntries(allObjects.map((name)=>[name,"ALL"])) });
