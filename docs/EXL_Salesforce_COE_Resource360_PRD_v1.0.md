...
|---|---|---|---|
| ADMUI-01 Administration landing | Authorized admins/Operations | Five-pillar Demo Activation Center for SSO, integrations, production-like fictional data, legal/business approvals and operational controls; run ID, 5/5 result, evidence links and safety boundary | Ready, passed, unauthorized and failed-check states; zero external calls; zero destructive actions; every run attributable. |
...
The implemented product is evidenced by the complete twelve-master live-interaction programme plus five explicitly labelled supplemental legacy micro-walkthroughs. Masters 01–11 cover all 103 governed Salesforce screens exactly once. Master 12 intentionally revisits decisive account, project, commercial, skill, staffing, allocation, budget, actual-time and certified-KPI states as a 17-stage executive golden path and closes by verifying the sanitized GitHub Pages publication lineage. Every master uses a branded opening coverage slide, specialized Salesforce screen behavior, continuous interaction footage, visible filtering/selection/evidence/outcomes, AAC narration, embedded English subtitles and WebVTT captions. The v3 recording QA register contains 106 interaction stages across Masters 03–12, including 89 previously unrecorded unique functional screens, and enforces frame diversity, stream format, poster and contact-sheet evidence. The five earlier 1280×720 Pages recordings remain integrity-checked supplemental evidence and are not counted toward master completion. File hashes, durations, source surfaces, screen coverage, personas and observed outcomes are maintained in `public/demo-videos/manifest.json` and `docs/DEMO_VIDEO_VALIDATION.md`; all recordings remain sanitized demo evidence and do not imply a live EXL connection.

The demo is not an EXL production go-live claim. It becomes the approved production delivery baseline only when the following governance actions are recorded; a reviewer may approve an assumption and later replace it through change control without reopening the whole PRD:
