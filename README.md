...
- 46 Salesforce data/config/event types (39 record objects including Account, six custom-metadata types and one platform event), 636 fields/formulas, 42 validation rules, nine guard triggers and native field history on material controls.
- 37 production Apex classes plus 13 focused test classes, including governed intake/commercial/work-plan/risk/closeout transactions, structured skill matching, deterministic golden-path and 10-account/20-project scale seeding, non-bypassable capacity control and chunked scope sharing.
