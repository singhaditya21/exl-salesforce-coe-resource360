...
The complete twelve-master narrated recording suite is published under `public/demo-videos/` for the sanitized Pages companion. Masters 01–11 cover all 103 governed Salesforce screens exactly once; Master 12 is an overlapping 17-stage executive golden path that traverses account, project, commercial, skills, staffing, allocation, budget, time, certified KPI and synchronized Pages evidence. Every master uses specialized screens and continuous filtering, selection, drill-down, action and outcome footage. Five earlier Pages micro-walkthroughs remain clearly labelled supplemental legacy evidence and are not counted toward master completion. The Pages recording cards derive durations and generation labels from the checked-in integrity manifest so the public catalogue cannot drift from deployed media. The [video validation catalog](docs/DEMO_VIDEO_VALIDATION.md) maps every recording to its source surface, executed screen coverage, outcome and demo boundary; CI verifies every MP4, poster, caption, contact sheet, stream and unique-state threshold. The immutable release baseline, persona order, reset controls, narration and acceptance criteria are defined in the [final demo recording storyboard](docs/DEMO_RECORDING_STORYBOARD.md).
