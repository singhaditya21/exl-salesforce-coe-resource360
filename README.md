...
The first two narrated v2.1 live-interaction masters and five clearly labelled legacy functional-evidence walkthroughs are published under `public/demo-videos/` for the sanitized Pages companion. Master 01 is a 3:07, 1920×1080 Salesforce recording covering `GLB-01` through `GLB-06`; Master 02 is a 3:40 authenticated Salesforce Engagement 360 recording covering `ENG-01` through `ENG-08` and the account-to-delivery chain. Both revised masters use specialized screens and continuous filtering, selection, drill-down, action and outcome footage. The Pages recording cards derive their durations and generation labels from the checked-in integrity manifest so the public catalogue cannot drift from the deployed media. The remaining masters follow the 103-screen coverage register. The [video validation catalog](docs/DEMO_VIDEO_VALIDATION.md) maps every recording to its source surface, executed screen coverage, outcome and demo boundary; CI verifies every MP4 against the checked-in integrity manifest. The tagged recording sequence, persona order, reset controls, narration points and post-production acceptance criteria are defined in the [final demo recording storyboard](docs/DEMO_RECORDING_STORYBOARD.md).
